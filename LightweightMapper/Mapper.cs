...
namespace LightweightMapper
{
    public static class Mapper<TSource, TTarget> where TSource : class where TTarget : class
    {
        ...
        private static Func<TSource, TTarget> GetMap()
        {
            ...
            var parameterExpression = Expression.Parameter(sourceType, "parameter");
        ...
        private static MemberInitExpression GetExpression(Expression parameterExpression, Type sourceType, Type targetType)
        {
            ...
            var listTargetTypeProperties = targetType.GetProperties().Where(x => x.PropertyType.IsPublic && x.CanWrite).ToList();
            foreach (var targetItem in listTargetTypeProperties)
            {
                ...
                if (targetItem.PropertyType.IsClass && sourceItem.PropertyType.IsClass && targetItem.PropertyType != sourceItem.PropertyType)
                {
                    ...
                    {
                        var memberInitExpression = GetExpression(propertyExpression, sourceItem.PropertyType, targetItem.PropertyType);
                        memberBindings.Add(Expression.Bind(targetItem, memberInitExpression));
                        continue;
                    }
                }
